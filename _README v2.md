...
2. Open **Terminal** and run:
   ...
   ollama pull mistral
   ```
   **Or** for the alternative model:
   ```bash
   ollama pull qwen2.5:14b
   ```
   This downloads the model (~4 GB for mistral, ~9 GB for qwen2.5:14b). You only do this once per model.
...
Citations appear as `[1]`, `[2]` etc. — bracket number only, no filename or page details inline.

### Sources cited
Sources appear **in the order they are cited in the answer** — the bracket numbers match the order of first appearance. Only sources actually referenced in the answer are shown.
...
OLLAMA_MODEL = "qwen2.5:14b"   # strong alternative with good citation accuracy
OLLAMA_MODEL = "llama3.2"      # another solid choice
...
| *"Model 'X' not found"* | Run `ollama pull <model_name>` in Terminal |
...
| No sources listed after answer | LLM did not cite inline — try `mistral` or `qwen2.5:14b`, increase `TOP_K`, or rephrase |
...
| Sources listed but not in citation order | This is expected — sources appear in the order they're first cited in the answer |
...
| Sources shown in citation order (order of first appearance) | ✅ |
...
| Multiple model support (mistral, qwen2.5:14b, llama3.2, phi3) | ✅ |
| Improved connection test with clear error messages | ✅ |
| Interactive Q&A loop (Block 8) | ✅ |

## 🔮 Future Ideas

- **Streamlit web interface** — GUI for easier use without Jupyter
